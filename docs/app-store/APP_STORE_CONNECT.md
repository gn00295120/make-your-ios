...
• Local voice notes with optional on-device-only transcription, editable review, and no network-recognition fallback
• Explicitly opted-in tiny apps that open from Apple Shortcuts or Siri through one authenticated foreground action
...
Projects, records, bookmarks, watchlists, ledgers, voice notes, accepted transcripts, and selected or captured results stay on your iPhone. Device access starts only after you tap, scanned URLs are not opened automatically, and there is no background audio, speech, location, or motion monitoring. Speech transcription requires an available on-device language model and never falls back to network recognition. Sharing leaves MakeYour only after you choose a destination in Apple's share sheet. Before an in-app AI request, MakeYour shows the exact task and text that will be sent. An accepted transcript may become editable text, but its audio is never attached. Photos, records, other apps, and device data are not attached automatically.

When generated versions include the visible Shortcuts access block, Apple's dynamic picker receives only opted-in tiny apps' stable IDs, display names, and validated icons. The fixed action requires local device authentication and opens MakeYour in the foreground. It receives no prompt, state, photos, voice notes, provider keys, or generated automation code. Removing the block or deleting a tiny app revokes that app's listing.
...
• 只留在本機的語音筆記，以及不會改走網路辨識、儲存前可編輯確認的裝置端逐字稿
• 經明確加入並確認後，可透過 Apple 捷徑或 Siri 驗證身分並在前景開啟的迷你 App
...
專案、紀錄、收藏、帳本、語音筆記、確認後的逐字稿與拍攝或選取的結果留在 iPhone。裝置功能只在點擊後啟動，掃描到的網址不會自動開啟，也沒有背景錄音、語音辨識、定位或動作監控。逐字稿只在裝置支援本機語言模型時執行，不會改走網路辨識；使用者可以先編輯再決定是否儲存。分享內容只有在你從 Apple 分享表單選擇目的地後才會離開 MakeYour。相機拍攝、即時掃描、錄音、裝置端語音辨識、計步與有意義的觸覺測試需要支援的實體 iPhone；不支援的裝置或模擬器會清楚顯示無法使用。

只有含有可見「捷徑存取」積木並經使用者確認的迷你 App 才會出現在 Apple 捷徑。系統只取得專案識別碼、顯示名稱與安全圖示；不會取得提示詞、狀態、媒體或 API key。固定動作要求裝置本機驗證，只能在前景開啟 MakeYour；移除積木或刪除迷你 App 後即失效。
